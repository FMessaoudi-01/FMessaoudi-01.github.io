...
- LLMNR used to identify hosts when DNS failed to do so, previously NBT-NS

- Keyflaw is that the service uses the user's username and their NTLMv2 hash 

### Tool:

Using the responder tool on linux, it’s going to listen and act as a mitm and get the windows machine into giving the hash then we try to crack it afterwards.
...
Let's first check if smb is disabled or not enforced:

```jsx
nmap --script=smb2-security-mode.nse --p445 @ip
```

Second we edit responder conf file and turn off smb and http then fire up responder using 
