...
- it’s used a lot for pivoting after gaining access to the AD so a compromised account is necessary. 
...
- we can also take the hash from a dumping tool like [secretsdump.py](http://secretsdump.py)
...
- another option is the —lsa which dumps lsa secrets and hackable ntlm hashes.
...
- first we create this in a powershell environment
