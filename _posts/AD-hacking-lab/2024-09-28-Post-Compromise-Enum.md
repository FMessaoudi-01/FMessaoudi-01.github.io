...
- Using a similar method as mitm6 but with ldap
...
- it’s going to generate some information by quering the domain, now we got to dig deeper using the reporting tasks.

```jsx
sudo python3 PlumHound.py -x tasks/default.tasks -p Neo4j@2024
```

- that will give us so much information and we should gather as much as we can while connecting the dots.