...
- *My writeups are my own unique mind-map than an "official" walkthrough.*
...
- it’s also shown that smb port is open and allows guest login.
...
- Now comes the real work, enumerating the domain users couldn’t be done with a bruteforce since we don’t have a wordlist so there’s a tool that did the trick.
...
- i immediately went back to the DEV share and bingo there’s a script with emily.oscars’ password.
...
- Not everything was straightforward, I struggled in the enumeration part because crackmapexec wasn’t working with the —rid-brute so I wasted time thinking the method was wrong.
- I also tried the kerberos_enumeration in metasploit to bruteforce using different username wordlists but that didn't work.
...
- it was an easy machine indeed, after i found the first username it was pretty straightforward.